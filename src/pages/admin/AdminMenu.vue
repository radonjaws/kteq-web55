...
onMounted(async () => {
  try {
    ...
    if (content.navLinks) {
      form.listen = content.navLinks.listen !== false
      form.donate = content.navLinks.donate !== false
    }
  } catch (e: any) {
...
async function handleSave() {
  const newSha = await save('content/settings.json', {
    ...rawContent.value,
    navLinks: {
      ...rawContent.value?.navLinks,   // preserve schedule, shows, djs, history, blog from Dashboard
      listen: form.listen,
      donate: form.donate,
    },
    ...
    <div class="mx-auto max-w-4xl px-4 py-8">
      ...
      <div class="flex items-start justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-kteq-muted">Visibility for nav items that don't have their own content section.</p>
        </div>
        <div class="flex items-center gap-3">
        ...
        <section class="rounded-lg border border-kteq-gray/30 bg-kteq-dark p-6">
          <div class="divide-y divide-kteq-gray/20">
            ...
            <div class="flex items-center justify-between py-3 first:pt-0">
              <div>
                <p class="font-display text-sm font-medium text-kteq-light">Listen Button</p>
                <p class="text-xs text-kteq-muted">The "Listen" play button in the site header</p>
              </div>
              <button
                type="button"
                @click="form.listen = !form.listen"
                ...
                :class="form.listen ? 'bg-kteq-yellow' : 'bg-kteq-gray'"
                role="switch"
                :aria-checked="form.listen"
              >
                <span
                  class="pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow transition-transform"
                  :class="form.listen ? 'translate-x-5' : 'translate-x-1'"
                />
              </button>
            </div>
            ...
            <div class="flex items-center justify-between py-3 last:pb-0">
              <div>
                <p class="font-display text-sm font-medium text-kteq-light">Donate</p>
                <p class="text-xs text-kteq-muted">/donate nav link</p>
              </div>
              <button
                type="button"
                @click="form.donate = !form.donate"
                class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-kteq-yellow/50 focus:ring-offset-2 focus:ring-offset-kteq-dark"
                :class="form.donate ? 'bg-kteq-yellow' : 'bg-kteq-gray'"
                role="switch"
                :aria-checked="form.donate"
              >
                <span
                  class="pointer-events-none inline-block h-4 w-4 rounded-full bg-white shadow transition-transform"
                  :class="form.donate ? 'translate-x-5' : 'translate-x-1'"
                />
              </button>
            </div>
