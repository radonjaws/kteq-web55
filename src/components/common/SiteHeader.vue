...
<template>
  <header class="sticky top-0 z-40 border-b border-kteq-gray/50 bg-kteq-black/95 backdrop-blur-md">
    <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4">
      <!-- Logo / Station ID -->
      <RouterLink to="/" aria-label="KTEQ — Home" class="group flex items-center gap-3" @click="mobileMenuOpen = false">
        <img v-if="logoUrl" :src="logoUrl" alt="KTEQ" class="h-9 w-auto object-contain shrink-0" />
        <div v-else aria-hidden="true" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-sm bg-kteq-yellow font-display text-sm font-bold tracking-tight text-kteq-black">
          K
        </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          ...
          @click="player.toggle()"
          :aria-label="player.isPlaying ? 'Stop listening' : 'Listen to KTEQ 91.3 FM'"
        ...
        >
          ...
          <span v-if="player.isPlaying" aria-hidden="true" class="flex items-end gap-[2px]">
          ...
          <svg v-else aria-hidden="true" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4">
        ...
        >
          <svg v-if="!mobileMenuOpen" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
          </svg>
          <svg v-else aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6">
    ...
    >
      <nav v-if="mobileMenuOpen" aria-label="Mobile navigation" class="border-t border-kteq-gray/50 bg-kteq-void px-4 py-4 md:hidden">
        <RouterLink
          v-for="link in navLinks"
          :key="link.to"
          :to="link.to"
